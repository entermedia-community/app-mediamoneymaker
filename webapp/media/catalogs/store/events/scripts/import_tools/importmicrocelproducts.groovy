			...
			if (!errorFields == true) {
				...
				while ((orderLine = read.readNext()) != null)
				{
					...
					if (manufacturer == null) {
						...
						if (manufacturer == null) {
							log.info("Manufacturer: " + orderLine[columnMicrocelManufacturerID] + " - INVALID");
						} else {
							log.info("Manufacturer: " + orderLine[columnMicrocelManufacturerID] + " - ADDED");
				...
				result.setErrorMessage("There seems to be a problem with the input file provided.");
				return result;
			}
		}
		finally
