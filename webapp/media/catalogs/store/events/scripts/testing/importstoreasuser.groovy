...
public class ImportStoreAsUser extends EnterMediaObject {
	
	ArrayList<String> goodStoreList;
	ArrayList<String> badStoreList;
	ArrayList<String> goodUserList;
	ArrayList<String> badUserList;
...
logs.startCapture();
try {
	ImportStoreAsUser importUsers = new ImportStoreAsUser();
	importUsers.setLog(logs);
	importUsers.setContext(context);
	importUsers.setModuleManager(moduleManager);
	importUsers.setPageManager(pageManager);
	importUsers.storeImport();
	context.putPageValue("goodlist", importUsers.getGoodStoreList());
	context.putPageValue("badlist", importUsers.getBadStoreList());
}
finally {
	logs.stopCapture();
}
