...
public class OrderSearcher extends BaseLuceneSearcher
{
	...
	public void reIndexAll(IndexWriter inWriter) {
		...
		try
		{
			buildIndex(newIndex,list);
			//restoreIndex(newIndex);
		}
		catch ( Exception ex)
		{
	...
	private void buildIndex(File inNewIndex, List inList) throws Exception {
		IndexWriter writer = new IndexWriter(FSDirectory.getDirectory(inNewIndex), false, getAnalyzer(), true);
		writer.setMergeFactor(100);
		writer.setMaxBufferedDocs(2000);

		PropertyDetails details = getPropertyDetailsArchive().getPropertyDetails("order");

		for (Iterator iterator = inList.iterator(); iterator.hasNext();)
		{
			OrderId id = (OrderId) iterator.next();
			SubmittedOrder order = getOrderArchive().loadSubmittedOrder(getStore(),id.getUsername(), id.getOrderId());
			if( order != null)
			{
			...
			{
				log.error("Could not load " + id.getUsername() + " " + id.getOrderId());
			}
		}
		writer.close();
	}
...
public List listOrders( Store inStore, WebPageRequest inContext ) throws StoreException
{
...
public HitTracker listOrdersForUser( Store inStore, User inUser ) throws StoreException
{
	File ordersFile = getOrdersDirectory(inStore);
	HitTracker 	allorders = search( "username:" + inUser.getUserName(), "date" );
	return allorders;
}
...
protected void updateIndex(Order inOrder, Document doc, PropertyDetails inDetails)
{
	doc.add(new Field("id", inOrder.getId(), Field.Store.YES, Field.Index.NO_NORMS));
	if( inOrder.getCustomer() != null)
	{
		doc.add(new Field("username", inOrder.getCustomer().getUserName(), Field.Store.YES, Field.Index.NO_NORMS));
	}
//	//String date = 
//	if( job.getName() != null)
//	{
//		doc.add(new Field("name", inOrder.get(), Field.Store.YES, Field.Index.TOKENIZED));
//	}
...
//	keywords.append(job.getName());
	...
	for (Iterator iterator = details.iterator(); iterator.hasNext();)
	{
		...
		else if (prop.equals("products"))
		{
			...
			for (Iterator iterator2 = inOrder.getItems().iterator(); iterator2.hasNext();)
			{
		...
		{
			value = DateTools.dateToString(inOrder.getDate(), Resolution.MINUTE);
		}
		else
		{
		...
		if( value != null)
		{
			if( detail.isStored())
			{
				...
				{
					doc.add( new Field(detail.getId(),value, Field.Store.YES, Field.Index.TOKENIZED));
				}
			}
			else
			{
				doc.add( new Field(detail.getId(),value, Field.Store.NO, Field.Index.TOKENIZED));
			}
			if( detail.isKeyword())
			{
				keywords.append(" ");
		...
		List productkeywords = getPropertyDetailsArchive().getPropertyDetailsCached("product").findKeywordProperties();
		for (Iterator iterator2 = inOrder.getItems().iterator(); iterator2.hasNext();)
		{
			CartItem item = (CartItem) iterator2.next();
			for (Iterator iterator3 = productkeywords.iterator(); iterator3.hasNext();)
			{
				PropertyDetail itemdetail = (PropertyDetail) iterator3.next();
		...
		doc.add(new Field("description", keywords.toString(), Field.Store.YES, Field.Index.TOKENIZED));
	}
}
...
public Object searchById(String inId) {
	...
	HitTracker tracker = search("id: " + inId);
	if( tracker.size() > 0)
	{
		Data hit = (Data)tracker.get(0);
		String username = hit.get("username");
		return getOrderArchive().loadSubmittedOrder(getStore(), inId, username);
	}
	return null;
}
